...
def print_version():
    ...
    if platform.system() == 'Windows':
        logger.info('Application Security Scanner %s', ver)
...
def check_update():
    try:
        ...
        if remote_version:
            ...
            if sem_loc < sem_rem:
                logger.warning('More exciting updates coming to AppScanX in near future, make sure to follow https://www.linkedin.com/in/syed-fawad-abbas/ and https://www.linkedin.com/company/apprisecyber/for more details, '
...
def get_device():
    ...
    logger.error('Is the Android VM running?\n'
                 'AppScanX cannot identify device id.\n'
...
def get_adb():
    ...
    try:
        ...
        if ADB_PATH:
            ...
            logger.warning('Dynamic Analysis will be '
                           'added in near future. '
                           '\nMake sure to follow Apprise Cyber Pvt Ltd.'
                           'https://www.linkedin.com/company/apprisecyber/'
...
def check_basic_env():
    """Check if we have basic env for AppScanX to run."""
    logger.info('AppScanX Basic Environment Check')
    try:
